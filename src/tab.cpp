...
int parse_tab_chord(void)
{
  ...
  char* tabdecos = "'UVX*#";  /*decos that may appear inside chord*/
  ...
  case FRENCH5TAB:
    for (j=0;j<s->npitch;j++) {
      ...
      if (s->pits[j] < 7) {
        ...
        bourdonwarning=1;
        wng("Warning: french5tab only supports 6 courses","");
      }
    }
    break;
  case FRENCH4TAB:
    for (j=0;j<s->npitch;j++) {
      ...
      if (s->pits[j] < 6) {
        ...
        bourdonwarning=1;
        wng("Warning: french4tab only supports 5 courses","");
      }
    }
    break;
  case SPANISHTAB:
  ...
  case GERMANTAB:
    {
      for (j=0;j<s->npitch;j++) {
        ...
        if (s->pits[j] < 7) {
          ...
          bourdonwarning=1;
          wng("Warning: diapasons are ignored in germantab","");
        }
      }
      break;
    }
